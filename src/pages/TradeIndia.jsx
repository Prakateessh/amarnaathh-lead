...
export default function TradeIndia() {
  ...
  const [dates, setDates] = useState(() => {
    ...
    return { start: yesterday.toISOString().split('T')[0], end: today.toISOString().split('T')[0] };
  });
  ...
  useEffect(() => { sessionStorage.setItem('ti_dates', JSON.stringify(dates)); }, [dates]);
  useEffect(() => { sessionStorage.setItem('ti_leads', JSON.stringify(leads)); }, [leads]);
  ...
  const handleFetch = async () => {
    ...
    try {
      ...
      if (result.total === 0) {
        setStatus({ type: 'error', message: '❌ Zero leads found for the selected date range.' });
      } else {
  ...
  const handleSendToCRM = async () => {
    ...
    try {
      const payload = qualifiedLeads.map(lead => ({
        ...
        company_name: lead.company, phone: lead.phone, location: lead.location,
        price: 0, source: "TradeIndia", notes: "[System] Auto-imported via TradeIndia API Integration."
      }));
  ...
  const toggleFilter = (value) => {
    setFilters(prev => ({ ...prev, status: prev.status.includes(value) ? prev.status.filter(v => v !== value) : [...prev.status, value] }));
  };
  ...
  const handleDownloadExcel = () => {
    ...
    const excelData = processedLeads.map(lead => ({
      'Date': formatDisplayDate(lead.date), 'Requirement': lead.requirement || '', 'Name': lead.name || '',
      'Company': lead.company || '', 'Phone': lead.phone || '', 'Location': lead.location || '', 'Status': lead.status || 'Pending'
    }));
    const worksheet = XLSX.utils.json_to_sheet(excelData);
    worksheet['!cols'] = [{ wch: 15 }, { wch: 40 }, { wch: 25 }, { wch: 30 }, { wch: 15 }, { wch: 25 }, { wch: 15 }];
        ...
        <div className="border-b border-slate-200 pb-8 flex flex-col gap-6">
          <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-8">
            <div className="w-full xl:w-1/2">
              ...
              <p className="text-slate-500 font-medium text-lg mt-3">Pull inbound requests directly from the TradeIndia vendor API.</p>
            </div>
                  ...
                  {processedLeads.length > 0 ? (
                    processedLeads.map((lead, idx) => (
                      <tr key={lead.id} className={`transition-colors duration-150 ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/70'} hover:bg-[#EBA7FF]/10`}>
                        <td className="py-5 px-6 text-slate-900 font-mono font-bold text-sm whitespace-nowrap">{formatDisplayDate(lead.date)}</td>
